class MangaCopy extends ComicSource {
    ...
    url = "https://cdn.jsdelivr.net/gh/istayheart/my-venera-sources@main/mangacopy_1_4_5.js"

    async getReqID(region = this.copyRegion) {
        if (region === "0") {
            return "";
        }
    ...
    get headers() {
        return this.buildHeaders(this.copyRegion);
    }

    buildHeaders(region = this.copyRegion) {
    ...
    parseJson(text) {
        ...
        try {
            ...
            if (start < 0) {
                throw `JSON parse failed: ${text.substring(0, 120)}`;
            }
    ...
    get deviceinfo() {
        ...
        if (!info) {
            info = MangaCopy.generateDeviceInfo();
    ...
    get pseudoid() {
        ...
        if (!pid) {
            pid = MangaCopy.generatePseudoid();
    ...
    account = {
        ...
        login: async (account, pwd) => {
            ...
            if (res.status === 200) {
                let data = this.parseJson(res.body)
    ...
    explore = [
        {
            ...
            load: async () => {
                ...
                let data = this.parseJson(dataStr.body)
    ...
    category = {
        ...
        parts: [
            ...
            },
            {
                name: "主题",
                type: "fixed",
                categories: Object.keys(MangaCopy.category_param_dict),
                categoryParams: Object.values(MangaCopy.category_param_dict),
                itemType: "category"
            }
        ]
    }
        ...
        load: async (category, param, options, page) => {
            ...
            if (category === "排行" || param === "ranking") {
                ...
                if (category !== undefined && category !== null) {
                    ...
                    param = MangaCopy.category_param_dict[category] || "";
                }
        ...
        optionList: [
            {
                ...
                notShowWhen: null,
                showWhen: Object.keys(MangaCopy.category_param_dict)
            },
            {
                options: [
        ...
        load: async (keyword, options, page) => {
            ...
            // 一般的搜索情况
                ...
                let search_url = this.loadSetting('search_api') === "webAPI"
                    ? `${this.apiUrl}${MangaCopy.searchApi}`
    ...
    favorites = {
        ...
        addOrDelFavorite: async (comicId, folderId, isAdding) => {
            ...
            let comic_id = this.parseJson(comicData.body).results.comic.uuid
            let res = await Network.post(
    ...
    comic = {
        loadInfo: async (id) => {
            ...
            let getFavoriteStatus = async (id, region) => {
                try {
                    let res = await Network.get(`${this.apiUrl}/api/v3/comic2/${id}/query`, this.buildHeaders(region));
                    if (res.status !== 200) {
                        return false;
                    }
                    let data = this.parseJson(res.body);
                    return data.results && data.results.collect != null;
                } catch (e) {
                    return false;
                }
            }
            let loadDetail = async (region) => {
                let reqId = await this.getReqID(region);
                return await Promise.all([
                    ...
                    getFavoriteStatus.bind(this)(id, region)
                ]);
            }

            let detailRegion = this.copyRegion;
            let results = await loadDetail(detailRegion);
            if (results[0].status === 210 && detailRegion === "0") {
                detailRegion = "1";
                results = await loadDetail(detailRegion);
            }

            if (results[0].status !== 200) {
                let message = `Invalid status code: ${results[0].status}`;
                try {
                    let errorData = this.parseJson(results[0].body);
                    message = errorData.results?.detail || errorData.message || message;
                } catch (e) {
                }
                throw message;
            }

            let data = this.parseJson(results[0].body).results;
            if (!data || !data.comic) {
                throw "漫画详情为空，请切换 CDN 线路或稍后重试";
            }
            ...
            let chapters = await getChapters(id, data.groups, detailRegion);
            ...
            while (attempt < maxAttempts) {
                try {
                    ...
                    if (res.status === 210) {
                        ...
                        try {
                            let responseBody = this.parseJson(res.body);
                            if (
    ...
    settings = {
        ...
        region: {
            ...
            default: MangaCopy.defaultCopyRegion,
        },
        image_quality: {
            title: "图片质量",
            type: "select",
        ...
        base_url: {
            ...
            default: MangaCopy.defaultApiUrl,
        },
        clear_device_info: {
            title: "清除设备信息",
            type: "callback",
        ...
        //     title: "拷贝版本（重启APP生效）",
        //     type: "input",
        //     default: MangaCopy.defaultCopyVersion,
        // },
        // platform: {
        //     title: "平台代号（重启APP生效）",
        //     type: "input",
        //     validator: '^\\d+(?:\\.\\d+)*$',
        //     default: MangaCopy.defaultCopyPlatform,
        // },
    }
    ...
    async refreshAppApi() {
        ...
        if (res.status === 200) {
            let data = this.parseJson(await res.text());
